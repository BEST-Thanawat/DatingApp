...
using ASPNET_Core_and_Angular.Helpers;
...
namespace ASPNET_Core_and_Angular.Controllers
{
    ...
    public class PhotosController : ControllerBase
    {
        ...
        public async Task<IActionResult> AddPhotoForUser(int userId, [FromForm] PhotoForCreationDto photoForCreationDto)
        {
            ...
            var userFromrRepo = await _repo.GetUser(userId);
            ...
            if (!userFromrRepo.Photos.Any(u => u.IsMain)) photo.IsMain = true;

            userFromrRepo.Photos.Add(photo);
            
            if (await _repo.SaveAll())
            {
                var photoToReturn = _mapper.Map<PhotoForReturnDto>(photo);
                return CreatedAtAction("GetPhoto", new { id = photo.Id }, value: photoToReturn);
